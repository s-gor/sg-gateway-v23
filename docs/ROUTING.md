...
## Внешние выходы и группы

На странице **Outbounds** можно добавить внешние выходы двух типов:

- SOCKS5;
- HTTP CONNECT.

SG-Gateway хранит реквизиты на сервере, собирает полный Xray candidate, выполняет `xray run -test` и только после успешной проверки применяет изменение. Пароли внешних выходов не выводятся обратно в интерфейс.

Созданный внешний выход появляется как отдельное действие в **Routing**. Правило может направить совпавший трафик через конкретный внешний outbound так же, как через SG-Gateway, WARP или Block.

Из нескольких внешних выходов можно создать группу:

- **Round Robin** — последовательное распределение новых соединений;
- **Random** — случайный выбор выхода;
- **Failover** — первый выход основной, второй резервный. Xray Observatory проверяет основной выход; при его недоступности используется `fallbackTag` резервного выхода.

Удаление выхода или группы блокируется, если объект используется активным Routing. Сначала нужно перевести правила на другой выход и применить Routing.
