...
def write_single_edge_constants() -> None:
    Path("app/single_edge.py").write_text(
        '''from __future__ import annotations
...
def patch_client_runtime() -> None:
    path = "hostd/sg_hostd/client_runtime.py"
    replace_once(
        path,
    ...
    replace_once(path, "while not _udp_port_listening(hysteria_profile.port):", "while not _udp_port_listening(HYSTERIA2_UDP_INTERNAL_PORT):")
    replace_once(
        path,
    ...
    replace_once(path, 'port = int(settings.get("anytls_port") or 9443)', "port = ANYTLS_TCP_INTERNAL_PORT")
    replace_once(path, 'port = int(settings.get("tuic_port") or 10443)', "port = TUIC_UDP_INTERNAL_PORT")
    ...
    if block.count('"listen": "::",') != 2:
        raise SystemExit(f"{path}: expected exactly two public sing-box listeners, got {block.count(chr(34) + 'listen' + chr(34) + ': ' + chr(34) + '::' + chr(34) + ',')}")
    block = block.replace('"listen": "::",', '"listen": "127.0.0.1",')
...
def patch_exports() -> None:
    ...
    replace_exact_count(path, "_format_endpoint(endpoint_host, settings.port)", "_format_endpoint(endpoint_host, PUBLIC_UDP_PORT)", 2)
    replace_exact_count(path, 'port = int(settings.config.get("mieru_port", settings.port or 2099))', "port = PUBLIC_TCP_PORT", 2)
    replace_once(path, 'transport = str(settings.config.get("mieru_transport", "TCP")).upper()', 'transport = "TCP"')
    replace_once(path, 'transport = str(settings.config.get("mieru_transport") or "TCP").upper()', 'transport = "TCP"')
    replace_once(path, 'endpoint = format_host_port(host, int(config.get("port", 9443)))', "endpoint = format_host_port(host, PUBLIC_TCP_PORT)")
    replace_once(path, 'endpoint = format_host_port(host, int(config.get("port", 10443)))', "endpoint = format_host_port(host, PUBLIC_UDP_PORT)")
    replace_once(
        path,
        '"security": "tls",\n            "sni": tls_domain,',
        '"security": "tls",\n            "sni": tls_domain,\n            "alpn": ANYTLS_ALPN,',
...
def patch_mihomo_service() -> None:
    path = "app/mihomo/service.py"
    p = Path(path)
    text = p.read_text(encoding="utf-8")
    anchor = "from app.maintenance.operations import log_operation\n"
    if text.count(anchor) != 1:
        raise SystemExit("mihomo import anchor mismatch")
    text = text.replace(
        anchor,
        anchor
    ...
    regex_once(path, r'"mieru_port": _int\(config\.get\("mieru_port"\), current\.port or 2099\),', '"mieru_port": PUBLIC_TCP_PORT,')
    regex_once(
        path,
        r'"mieru_transport": \(\n\s+"UDP"\n\s+if str\(form\.get\("mieru_transport", "TCP"\)\)\.upper\(\) == "UDP"\n\s+else "TCP"\n\s+\),',
        '"mieru_transport": "TCP",',
    )
    regex_once(path, r'"anytls_port": _int\(config\.get\("anytls_port"\), 8443\),', '"anytls_port": PUBLIC_TCP_PORT,')
    regex_once(path, r'"tuic_port": _int\(config\.get\("tuic_port"\), 10443\),', '"tuic_port": PUBLIC_UDP_PORT,')
    regex_once(path, r'"mieru_port": _int\(config\.get\("mieru_port"\), settings\.port or 2099\),', '"mieru_port": PUBLIC_TCP_PORT,')
    regex_once(
        path,
        r'"mieru_transport": \(\n\s+"UDP"\n\s+if str\(config\.get\("mieru_transport", "TCP"\)\)\.upper\(\) == "UDP"\n\s+else "TCP"\n\s+\),',
        '"mieru_transport": "TCP",',
    )
    regex_once(path, r'"anytls_port": _int\(config\.get\("anytls_port"\), 8443\),', '"anytls_port": PUBLIC_TCP_PORT,')
    regex_once(path, r'"tuic_port": _int\(config\.get\("tuic_port"\), 10443\),', '"tuic_port": PUBLIC_UDP_PORT,')
    ...
    regex_once(
        path,
        r'\n    ports: list\[tuple\[str, int\]\] = \[\].*?raise MihomoError\("Порты Mieru, AnyTLS и TUIC должны различаться"\)\n',
        "\n",
        flags=re.S,
    )
    regex_once(
        path,
        r'\n    awg = get_connection_settings\("amneziawg"\).*?raise MihomoError\(\n                f"\{label\}: порт \{port\}/\{transport\} уже используется: \{conflict\}"\n            \)\n',
        "\n",
        flags=re.S,
    )
    ...
    # Public device YAML always advertises the shared edge.
    replace_once(path, 'f"    port: {settings[\'mieru_port\']}"', 'f"    port: {PUBLIC_TCP_PORT}"')
    replace_once(path, 'f"    transport: {settings[\'mieru_transport\']}"', '"    transport: TCP"')
    replace_once(path, 'f"    port: {settings[\'anytls_port\']}"', 'f"    port: {PUBLIC_TCP_PORT}"')
    replace_once(path, 'f"    port: {settings[\'tuic_port\']}"', 'f"    port: {PUBLIC_UDP_PORT}"')
    replace_once(
        path,
...
def patch_mihomo_template() -> None:
    path = "app/web/templates/_mihomo_panel.html"
    regex_once(
        path,
        r'\n        <div class="mhv2-basic-fields one">.*?name="mieru_transport".*?</div>',
        '\n        <p class="mhv2-runtime-note">Single Edge: Mieru использует TCP/443.</p>',
        flags=re.S,
    )
...
def patch_installer() -> None:
    path = "install.sh"
    replace_once(path, 'MIHOMO_PORT="2099"', 'MIHOMO_PORT="10448"')
    replace_once(path, 'HYSTERIA2_PORT="8446"', 'HYSTERIA2_PORT="10452"')
    replace_once(path, 'ANYTLS_PORT="9443"', 'ANYTLS_PORT="10449"')
    replace_once(path, 'TUIC_PORT="10443"', 'TUIC_PORT="10453"')
    replace_once(path, 'PLACEHOLDER_TLS_INTERNAL_PORT="7444"', 'PLACEHOLDER_TLS_INTERNAL_PORT="7444"\nUDP_EDGE_SERVICE="sg-gateway-udp-edge.service"')
    replace_once(
        path,
    ...
    regex_once(
        path,
        r'for rule in \\\n\s+"80/tcp" "\$\{XRAY_PORT\}/tcp" "\$\{XRAY_PORT\}/udp" \\\n\s+"\$\{HYSTERIA2_PORT\}/udp" \\\n\s+"\$\{MIHOMO_PORT\}/tcp" "\$\{ANYTLS_PORT\}/tcp" "\$\{TUIC_PORT\}/udp"; do',
        'for rule in \\\n      "80/tcp" "443/tcp" "443/udp"; do',
    )

    p = Path(path)
    text = p.read_text(encoding="utf-8")
    marker = "stage_firewall_and_network() {"
    if text.count(marker) != 1:
        raise SystemExit(f"install stage_firewall marker count={text.count(marker)}")
    block = '''install_udp_edge_service() {
...
'''
    ...
    text = text.replace("stage_firewall_and_network() {\n", "stage_firewall_and_network() {\n  install_udp_edge_service\n", 1)
    p.write_text(text, encoding="utf-8", newline="\n")


def write_udp_edge() -> None:
    root = Path("app/udp_edge")
    root.mkdir(parents=True, exist_ok=True)
    (root / "__init__.py").write_text('"""SG-Gateway public UDP/443 dispatcher."""\n', encoding="utf-8", newline="\n")
    (root / "classifier.py").write_text(
...
''',
    ...
    Path("deploy/sg-gateway-udp-edge.service").write_text(
        '''[Unit]
Description=SG-Gateway UDP/443 protocol dispatcher
After=network-online.target xray.service sg-gateway-awg31.service sg-gateway-singbox.service
...
User=sg-gateway
Group=sg-gateway
WorkingDirectory=/opt/sg-gateway
Environment=PYTHONPATH=/opt/sg-gateway
ExecStart=/opt/sg-gateway/.venv/bin/python -m app.udp_edge.server
Restart=on-failure
RestartSec=1
...
def main() -> None:
    write_single_edge_constants()
    patch_client_runtime()
    patch_exports()
    patch_mihomo_service()
    patch_mihomo_template()
    patch_installer()
    write_udp_edge()
    print("single-edge patch batch applied")
