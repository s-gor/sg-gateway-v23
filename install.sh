...
require_supported_ubuntu() {
  if [[ ! -r /etc/os-release ]]; then
    echo "Не удалось определить операционную систему. Требуется Ubuntu 24.x–26.x." >&2
    exit 1
  fi
  # shellcheck disable=SC1091
  . /etc/os-release
  if [[ "${ID:-}" != "ubuntu" ]]; then
    printf 'Требуется Ubuntu 24.x–26.x. Обнаружено: %s\n' "${PRETTY_NAME:-неизвестная система}" >&2
    exit 1
  fi
  if [[ ! "${VERSION_ID:-}" =~ ^(24|25|26)\. ]]; then
    printf 'Поддерживаются Ubuntu 24.x–26.x. Обнаружено: %s\n' "${PRETTY_NAME:-Ubuntu ${VERSION_ID:-неизвестно}}" >&2
    exit 1
  fi
