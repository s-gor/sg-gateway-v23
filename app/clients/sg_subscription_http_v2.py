    ...
    if PUBLIC_ENDPOINT not in app.view_functions:
        def feed(token: str):
            ...
            if request.args.get("format", "").strip().lower() == "json":
                ...
                response = Response(build_sg_subscription_text(client), content_type="text/plain; charset=utf-8")
        ...
        app.add_url_rule(
            "/sg/sub/v1/<token>", endpoint=PUBLIC_ENDPOINT, view_func=feed, methods=["GET"]
        )
