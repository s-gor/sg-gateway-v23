...
def test_recovery_clean_install_requires_exact_sha_for_non_default_branch():
    ...
    assert 'non-default branch requires SG_GATEWAY_SOURCE_COMMIT' in installer
    assert 'installer is pinned to dev-02301' not in installer
...
def test_recovery_clean_install_uses_generic_archive_and_actual_channel():
    ...
    assert "printf '[SG-Gateway] Stable channel: %s\\n' \"$BRANCH\"" in installer
