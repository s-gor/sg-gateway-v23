...
def test_clean_installer_checks_free_space_before_download_and_extraction():
    ...
    assert 'require_free_space /tmp' not in text
    assert 'require_free_space /opt "installation and temporary storage"' in text
    assert "not enough free disk space for clean install" in text

    first_preflight = text.index('require_free_space /opt "installation and temporary storage"')
