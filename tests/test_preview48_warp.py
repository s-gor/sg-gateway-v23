...
def test_routing_ui_assigns_warp_but_management_is_in_outbounds():
    ...
    assert "confirm(" not in source + outbounds
    assert "alert(" not in source + outbounds
    assert "prompt(" not in source + outbounds
